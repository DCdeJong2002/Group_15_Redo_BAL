...
input_file = Path(
    r"C:\Users\douwe\AE4115-23_EXPERIMENTAL SIMULATIONS\Group_15_Redo_BAL\MODEL_OFF_DATA\modelOffData.xlsx"
)
...
aos_df.columns = ["AoA [deg]", "AoS [deg]", "CD", "CY", "CL", "CMroll", "CMpitch", "CMyaw"]

# Left block uses "Cy" in the sheet; normalize to "CY"
aoa_df = aoa_df.rename(columns={"Cy": "CY"})
aos_df = aos_df.rename(columns={"Cy": "CY"})
...
for df in [aoa_df, aos_df]:
    for col in ["AoA [deg]", "AoS [deg]", "CD", "CY", "CL", "CMroll", "CMpitch", "CMyaw"]:
...
coeff_cols = ["CD", "CY", "CL", "CMroll", "CMpitch", "CMyaw"]
