...
class BaseCorrector:
    ...
    def compute_ct_from_propon_propoff(
        ...
        ct_col_on: str = "CT",
        ct_col_off: str = "CT",
        match_cols = ("AoA_round", "AoS_round", "dR", "dE", "V_round"),
        new_col: str = "CT_prop",
    ):
        """
        Compute thrust coefficient from prop-on minus prop-off CT.

        The method matches cases between prop-on and prop-off datasets and
        computes:

            CT_prop = CT_prop_on - CT_prop_off

        Parameters
        ----------
        propoff_df : pd.DataFrame
            Dataframe containing prop-off data.
        ct_col_on : str
            CT column in prop-on dataset.
        ct_col_off : str
            CT column in prop-off dataset.
        match_cols : tuple
            Columns used to match operating conditions.
        new_col : str
            Name of the new thrust coefficient column.

        Returns
        -------
        pd.DataFrame
            Updated prop-on dataframe with CT_prop column.
        """
        ...
        self.require_columns(df_on, list(match_cols) + [ct_col_on], context="prop-on CT calculation")
        self.require_columns(df_off, list(match_cols) + [ct_col_off], context="prop-off CT calculation")

        # Prepare reduced off dataframe
        df_off_small = df_off[list(match_cols) + [ct_col_off]].rename(
            columns={ct_col_off: "CT_off"}
        )
        ...
        df[new_col] = df[ct_col_on] - df["CT_off"]

        # Diagnostic
        df["CT_prop_found"] = df["CT_off"].notna()
